...
export default function CaracteristicasSection() {
  return (
    <section id="caracteristicas" className="scroll-mt-28 md:scroll-mt-32 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {DIMENSIONES.map((dim, idx) => {
          ...
          return (
            <div
              key={dim.titulo}
              className={`p-6 rounded-2xl bg-white border border-slate-200/80 shadow-md shadow-slate-100 hover:shadow-xl hover:border-blue-400 transition-all duration-300 flex flex-col justify-between group ${
                idx === 0 ? 'lg:col-span-1 md:col-span-2 lg:row-span-1 bg-gradient-to-br from-white via-blue-50/30 to-blue-50/20 border-blue-200' : ''
              }`}
            >
              <div className="space-y-3.5">
                <div className="flex items-center justify-between">
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform">
                    <Icon className="w-5 h-5" />
                  </div>
                  <span className="text-sm font-mono font-bold text-blue-800 px-2.5 py-1 rounded-full bg-blue-50 border border-blue-200">
                    Dimensión 0{idx + 1}
                  </span>
                </div>
                <h3 className="text-xl font-bold text-brand-navy font-display group-hover:text-blue-700 transition-colors">
                  {dim.titulo}
                </h3>
              ...
              <div className="mt-5 pt-3 border-t border-slate-100 flex items-center text-xs font-mono text-blue-700 font-semibold gap-1.5">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-blue-500/10 via-blue-500/5 to-white border border-blue-200/80 shadow-sm space-y-3">
        <h4 className="text-base font-bold text-brand-navy flex items-center gap-2 font-display">
          <Lightbulb className="w-5 h-5 text-blue-600 shrink-0" /> Carácter Transversal en la Vida Cívica y Profesional
        </h4>
