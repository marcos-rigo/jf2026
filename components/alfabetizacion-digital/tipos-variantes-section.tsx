...
import { Layers, Globe, MapPin, BarChart3, Bot, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export default function TiposVariantesSection() {
  return (
    <section id="tipos-variantes" className="scroll-mt-28 md:scroll-mt-32 space-y-10">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {NIVELES_BRECHA.map((item, idx) => (
            <div
              key={item.nivel}
              className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-md shadow-slate-100 hover:shadow-xl hover:border-sky-300 transition-all flex flex-col justify-between"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="w-9 h-9 rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white font-mono font-bold text-xs shadow-md shadow-sky-500/20">
                    0{idx + 1}
                  </span>
                  <span className="text-[0.7rem] font-mono text-sky-800 font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-sky-50 border border-sky-200">
                    {idx === 0 ? 'Físico' : idx === 1 ? 'Operativo' : 'Sustantivo'}
                  </span>
                </div>
                <h4 className="text-base font-bold text-slate-900 font-display">{item.nivel}</h4>
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-gradient-to-br from-white via-sky-50/40 to-blue-50/30 border border-sky-200/80 shadow-lg shadow-sky-500/5 space-y-6">
        ...
        <p className="text-sm text-slate-700 leading-relaxed font-sans">
          Estructura de referencia internacional consolidada en <strong className="text-slate-900 font-semibold">5 áreas competenciales principales</strong>, integrando de forma transversal competencias en <strong className="text-sky-800 font-semibold">Inteligencia Artificial</strong> en 4 niveles de proficiencia (Básico, Intermedio, Avanzado y Altamente Avanzado).
        </p>

        {/* Las 5 Áreas */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 pt-1">
          {DIGCOMP_AREAS.map((area, idx) => (
            <div key={area} className="p-4 rounded-xl bg-white border border-sky-200/60 shadow-sm space-y-1.5">
              <span className="text-[0.65rem] font-mono text-sky-600 font-bold block">ÁREA 0{idx + 1}</span>
              <p className="text-xs font-semibold text-slate-800 leading-snug">{area}</p>
            </div>
          ))}
        </div>

        {/* Integración Transversal de la IA */}
        <div className="p-4.5 rounded-2xl bg-sky-50 border border-sky-200/80 flex items-start gap-3.5">
          <Bot className="w-5 h-5 text-sky-600 shrink-0 mt-0.5" />
          <div className="text-xs text-slate-700 leading-relaxed space-y-1">
            <p className="font-bold text-sky-950 font-display">
              Integración de IA en DigComp 3.0: Habilidades AI-E (Explícitas) y AI-I (Implícitas)
            </p>
            <p>
              Distingue entre la interacción directa mediante prompt engineering y comprensión algorítmica (<em className="text-slate-900 font-semibold">AI-E</em>) y el pensamiento crítico frente a contenidos sintéticos o decisiones automatizadas mediadas por IA (<em className="text-slate-900 font-semibold">AI-I</em>).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-gradient-to-br from-white via-indigo-50/40 to-sky-50/30 border border-indigo-200/80 shadow-lg shadow-indigo-500/5 space-y-4">
        <div className="flex items-center gap-2 text-indigo-700 font-mono text-xs uppercase tracking-wider font-semibold">
        ...
        <div className="p-5 rounded-2xl bg-white border border-indigo-200/60 shadow-sm space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-700">
          <div className="p-4.5 rounded-xl bg-white border border-slate-200/80 space-y-1 shadow-sm">
            <span className="font-bold text-sky-700 block text-sm font-display">10 Niveles Granulares en 5 Categorías</span>
          ...
          <div className="p-4.5 rounded-xl bg-amber-50/60 border border-amber-200 space-y-1 shadow-sm">
            <span className="font-bold text-amber-900 block text-sm font-display">El Valor del Nivel Prebásico (Niveles 1 y 2)</span>
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-white border border-slate-200/80 shadow-md shadow-slate-100 space-y-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 text-amber-700 font-mono text-xs uppercase tracking-wider font-semibold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="p-5 rounded-2xl bg-sky-50/50 border border-sky-200/80 space-y-2 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 font-display">Habilidades Básicas</span>
              <span className="text-2xl font-black text-sky-600 font-mono">54%</span>
            </div>
          ...
          <div className="p-5 rounded-2xl bg-blue-50/50 border border-blue-200/80 space-y-2 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 font-display">Habilidades Intermedias</span>
              <span className="text-2xl font-black text-blue-600 font-mono">19%</span>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed">
          ...
          <div className="p-5 rounded-2xl bg-rose-50/50 border border-rose-200/80 space-y-2 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 font-display">Habilidades Avanzadas</span>
