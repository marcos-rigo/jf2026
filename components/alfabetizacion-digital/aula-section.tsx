...
import {
  ...
  AULA_INTRO,
  AULA_EJE_LABEL,
  AULA_COMUNIDAD_TITULO,
  AULA_COMUNIDAD_PARRAFO,
...
export default function AulaSection() {
  ...
  const eyebrow = resolveTexto(AULA_EYEBROW, audienciaActual, 'docentes');
  const titulo = resolveTexto(AULA_TITULO, audienciaActual, 'docentes');
  const intro = resolveTexto(AULA_INTRO, audienciaActual, 'docentes');
  const ejeLabel = resolveTexto(AULA_EJE_LABEL, audienciaActual, 'docentes');
  const comunidadTitulo = resolveTexto(AULA_COMUNIDAD_TITULO, audienciaActual, 'docentes');
  const comunidadParrafo = resolveTexto(AULA_COMUNIDAD_PARRAFO, audienciaActual, 'docentes');
  ...
  return (
    <section id="aula" className="scroll-mt-28 md:scroll-mt-32 space-y-8">
      ...
      {/* Grid de los 3 Ejes en Light Mode */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {AULA_PUNTOS.map((punto, idx) => {
          const Icon = iconsMap[idx] || GraduationCap;
          const puntoTitulo = resolveTexto(punto.titulo, audienciaActual, 'docentes');
          const puntoDesc = resolveTexto(punto.desc, audienciaActual, 'docentes');
          return (
            <div
              key={puntoTitulo}
